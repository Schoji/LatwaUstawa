import 'package:flutter/material.dart';
import 'package:latwe_ustawy/global/notifiers.dart';
import 'package:latwe_ustawy/home/home_page.dart';
import 'package:latwe_ustawy/members/member_builder.dart';

void main() {
  runApp(const MyApp());
}

class MyApp extends StatelessWidget {
  const MyApp({super.key});

  // This widget is the root of your application.
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'LatwaUstawa',
      debugShowCheckedModeBanner: false,
      theme: ThemeData(
        fontFamily: 'Geist',
        scaffoldBackgroundColor: const Color.fromARGB(255, 244, 244, 244),
        colorScheme: ColorScheme.fromSeed(
          seedColor: Colors.white,
          primary: Colors.red,
          surfaceContainerLow: Colors.white,
          surfaceContainerHigh: const Color.fromARGB(255, 250, 250, 250),
          onSurfaceVariant: const Color.fromARGB(255, 43, 43, 43),
          primaryContainer: Colors.blue,
          onPrimaryFixed: Colors.amber.shade800,
          onPrimaryFixedVariant: Colors.red,
          onSecondaryFixedVariant: Colors.green,
        ),
      ),
      darkTheme: ThemeData.dark().copyWith(
        textTheme: ThemeData.dark().textTheme.apply(fontFamily: 'Geist'),
        scaffoldBackgroundColor: Colors.black,
        colorScheme: ColorScheme.dark().copyWith(
          primary: Colors.red.shade300,
          surfaceContainerLow: Color.fromARGB(255, 13, 13, 13),
          surfaceContainerHigh: Color.fromARGB(255, 21, 21, 21),
          onPrimaryContainer: Color.fromARGB(13, 72, 72, 72),
          onSurfaceVariant: Colors.white.withAlpha(180),
          primaryContainer: Colors.blue.shade300,
          onPrimaryFixed: Colors.yellow.shade300,
          onPrimaryFixedVariant: Colors.red.shade400,
          onSecondaryFixedVariant: Colors.green.shade300,
        ),
        appBarTheme: AppBarTheme(backgroundColor: Colors.black),
      ),

      home: HomePage(),
    );
  }
}
